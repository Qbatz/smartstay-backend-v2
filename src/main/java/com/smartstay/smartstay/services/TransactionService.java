...
public class TransactionService {
    ...
    public ResponseEntity<?> recordPaymentForBooking(String hostelId, String invoiceId, AddPayment payment) {
        ...
        if (response == 1) {
            ...
            TransactionDto transaction = new TransactionDto(payment.bankId(), payment.referenceId(), payment.amount(),
    ...
    public List<PartialPaidInvoiceInfo> getTransactionInfo(List<String> partialPaymentInvoices) {
        return transactionRespository.findByInvoiceIdIn(partialPaymentInvoices).stream()
                .map(item -> new PartialPaidInvoiceInfo(item.getInvoiceId(), item.getPaidAmount())).toList();
    }
    ...
    public ResponseEntity<?> getReceiptDetailsByTransactionId(String hostelId, String transactionId) {
            ...
            if (!hostelTemplates.isMobileCustomized()) {
                ...
                if (invoicesV1.getInvoiceType().equalsIgnoreCase(InvoiceType.ADVANCE.name())
                    ...
                    hostelPhone = hostelTemplates.getTemplateTypes().stream()
                            ...
                            .map(BillTemplateType::getReceiptPhoneNumber).toList().getFirst();
                } else {
                    hostelPhone = hostelTemplates.getTemplateTypes().stream()
            ...
            if (!hostelTemplates.isEmailCustomized()) {
                ...
                if (invoicesV1.getInvoiceType().equalsIgnoreCase(InvoiceType.ADVANCE.name())
                    ...
                    hostelEmail = hostelTemplates.getTemplateTypes().stream()
                            .filter(item -> item.getInvoiceType().equalsIgnoreCase(BillConfigTypes.ADVANCE.name()))
                            .map(BillTemplateType::getReceiptMailId).toList().getFirst();
                } else {
                    hostelEmail = hostelTemplates.getTemplateTypes().stream()
                            .filter(item -> item.getInvoiceType().equalsIgnoreCase(BillConfigTypes.RENTAL.name()))
                            .map(BillTemplateType::getReceiptMailId).toList().getFirst();
        ...
        ReceiptInfo receiptInfo = new ReceiptInfo(transactionV1.getTransactionReferenceId(),
                transactionV1.getTransactionId(), Utils.dateToString(transactionV1.getPaymentDate()),
                Utils.dateToTime(transactionV1.getPaymentDate()), transactionV1.getPaidAmount(), invoiceType,
                transactionV1.getReferenceNumber(), receiverfullName.toString(), invoiceMonth);
    ...
    public ResponseEntity<?> deleteReceipt(String hostelId, String receiptId) {
        ...
        if (response == 1) {
            ...
            TransactionDto transaction = new TransactionDto(transactionV1.getBankId(),
                    transactionV1.getReferenceNumber(), transactionV1.getPaidAmount(), BankTransactionType.DEBIT.name(),
                    BankSource.INVOICE.name(), hostelId, null, transactionV1.getTransactionId());
    ...
    public TransactionReportResponse getTransactionReport(String hostelId, String period, String customStartDate,
        ...
        List<TransactionReportResponse.TransactionData> dataList = transactions.stream().map(t -> {
            ...
            if (t.getCustomerId() != null) {
                CustomersBedHistory bedHistory = customersBedHistoryService.getCustomerBedByStartDate(t.getCustomerId(),
                        t.getPaidAt(), t.getPaidAt());
                if (bedHistory != null) {
                    BedDetails bedDetails = bedService.getBedDetails(bedHistory.getBedId());
                    if (bedDetails != null) {
            ...
            return TransactionReportResponse.TransactionData.builder().receiptNo(t.getTransactionReferenceId())
                    .type(inv != null ? Utils.capitalize(inv.getInvoiceType())
                            : (t.getType() != null ? Utils.capitalize(t.getType()) : null))
                    .amount(inv != null ? inv.getTotalAmount() : 0.0).paymentMade(t.getPaidAmount())
                    .collectedBy(u != null
                            ? Utils.capitalize(
                                    u.getFirstName() + " " + (u.getLastName() != null ? u.getLastName() : ""))
                            : "Unknown")
                    .bankAccount(b != null ? b.getAccountHolderName() + "-" + Utils.capitalize(b.getAccountType())
                            : "Unknown")
                    .customerName(c != null
                            ? Utils.capitalize(
                                    c.getFirstName() + " " + (c.getLastName() != null ? c.getLastName() : ""))
                            : "Unknown")
                    .bed(bedName).room(roomName).floor(floorName)
                    .invoiceNumber(inv != null ? inv.getInvoiceNumber() : "N/A")
    ...
    private List<TransactionReportResponse.FilterOption> buildPaymentModeFilters() {
        return Arrays.stream(BankAccountType.values()).map(type -> TransactionReportResponse.FilterOption.builder()
                .id(type.name()).label(Utils.capitalize(type.name())).build()).collect(Collectors.toList());
    }
    ...
    private List<TransactionReportResponse.UserFilterOption> buildCollectedByFilters(String hostelId) {
        ...
        if (creatorIds != null && !creatorIds.isEmpty()) {
            usersService.findAllUsersFromUserId(creatorIds)
                    .forEach(u -> options.add(new TransactionReportResponse.UserFilterOption(u.getUserId(), Utils
                            .capitalize(u.getFirstName() + " " + (u.getLastName() != null ? u.getLastName() : "")))));
        }
        return options;
    }

    private TransactionReportResponse buildEmptyTransactionResponse(String hostelId, Date startDate, Date endDate,
            int page, int size) {
        return TransactionReportResponse.builder().status(true).message("Receipts report fetched successfully")
                .summary(TransactionReportResponse.Summary.builder().hostelId(hostelId)
                        .startDate(Utils.dateToString(startDate)).endDate(Utils.dateToString(endDate))
    ...
    public ResponseEntity<?> getAllReceiptsByHostelIdNew(String hostelId, String keyword, List<String> bankIds, Integer pageSize, Integer pageNumber, String period) {
        ...
        if (period != null) {
            if (period.equalsIgnoreCase("THIS_MONTH")) {
                Calendar cal = Calendar.getInstance();
            ...
            if (period.equalsIgnoreCase("LAST_MONTH")) {
                ...
                startDate = cal.getTime();
                endDate = Utils.findLastDate(1, cal.getTime());
            }
            if (period.equalsIgnoreCase("LAST_3_MONTHS")) {
                Calendar cal = Calendar.getInstance();
                cal.set(Calendar.DAY_OF_MONTH, 1);
                cal.add(Calendar.MONTH, -3);
            ...
            if (period.equalsIgnoreCase("LAST_6_MONTHS")) {
                Calendar cal = Calendar.getInstance();
                cal.set(Calendar.DAY_OF_MONTH, 1);
                cal.add(Calendar.MONTH, -6);
        ...
        if (listAllTransactions != null) {
            ...
            paidAmount = listAllTransactions
                    .stream()
                    .filter(i -> i.getType() == null)
                    .mapToDouble(i -> {
                        if (i.getPaidAmount() != null) {
        ...
        int totalReceipts = (int) listPagebleTransactions.getTotalElements();
        int currentPage = listPagebleTransactions.getPageable().getPageNumber() + 1;
        int totalPages = listPagebleTransactions.getTotalPages();
