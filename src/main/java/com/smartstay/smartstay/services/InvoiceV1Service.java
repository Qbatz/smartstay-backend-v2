...
import com.smartstay.smartstay.Wrappers.settlement.CurrentRentBreakUp;
...
import com.smartstay.smartstay.responses.discount.InvoiceDiscount;
import com.smartstay.smartstay.responses.invoices.CustomerDetails;
...
public class InvoiceV1Service {
    ...
    @Autowired
    public void setRoomsService(@Lazy RoomsService roomsService) {
        this.roomsService = roomsService;
    }

    @Autowired
    public void setFloorsService(@Lazy FloorsService floorsService) {
    ...
    public void addAdvanceInvoice(String customerId, Double amount, String type, String hostelId, String customerMobile, String customerMailId, String joiningDate, BillingDates billingDates, double deductionAmount, List<Deductions> listDeductions) {
        if (authentication.isAuthenticated()) {
            ...
            invoicesV1.setBasePrice(amount - deductionAmount);
            invoicesV1.setSubTotal(baseAmount);
    ...
    public String addBookingInvoice(String customerId, Double amount, String type, String hostelId, String customerMobile, String customerMailId, String bankId, String referenceNumber, Date bookingDate) {
        if (authentication.isAuthenticated()) {
            ...
            invoicesV1.setBalanceAmount(amount);
            invoicesV1.setInvoiceType(type);
            invoicesV1.setCustomerId(customerId);
            invoicesV1.setInvoiceNumber(invoiceNumber.toString());
            ...
            invoicesV1.setCreatedBy(authentication.getName());
            invoicesV1.setInvoiceDate(bookingDate);
    ...
    public ResponseEntity<?> getAllInvoices(String hostelId, String startDate, String endDate, List<String> types, List<String> createdBy, List<String> mode, String searchKey, List<String> paymentStatus, Integer pageFromParams, Integer size) {
        ...
        if (!listAllInvoice.isEmpty()) {
            ...
            List<String> invoicesId = listAllInvoice
                    ...
                    .map(InvoicesV1::getInvoiceId)
                    .toList();
            listTransactions = transactionService.getTransactionsByInvoiceIds(invoicesId);
...
//            List<InvoicesV1> listInvoices = invoicesV1Repository.findAllInvoicesByHostelId(hostelId, dStartDate, dEndDate, invoiceTypes, createdByUsers, modes, pStatus, userIds);
    ...
    public ResponseEntity<?> getAllInvoicesWebResponse(String hostelId, InvoiceFilterOptions filterOptions, Page<InvoicesV1> pageInvoices, List<InvoicesList> invoicesLists, List<InvoicesV1> listInvoicesForSummary, Double currentMonthCollection) {
        ...
        if (pageInvoices != null) {
            int totalPages = pageInvoices.getTotalPages();
            totalInvoices = (int) pageInvoices.getTotalElements();
            int currentPage = pageInvoices.getPageable().getPageNumber();
            int pageSize = pageInvoices.getPageable().getPageSize();

            paginationSummary = new PaginationSummary(totalPages,
                    totalInvoices,
                    currentPage + 1,
                    pageSize);
        }
    ...
    public InvoiceSummaryInfo getSummary(String hostelId, Integer totalInvoices, List<InvoicesV1> invoiceList, double currentMonthCollection) {
        ...
        double pendingRefundInvoices = validInvoices
                .stream()
                .filter(i -> i.getPaymentStatus().equalsIgnoreCase(PaymentStatus.PENDING_REFUND.name()) || i.getPaymentStatus().equalsIgnoreCase(PaymentStatus.PARTIAL_REFUND.name()))
                .mapToDouble(i -> {
                    double pAmount = 0.0;
                    ...
                    if (i.getTotalAmount() < 0) {
                        return (i.getTotalAmount() + pAmount) * -1;
                    }
                    return i.getTotalAmount() - pAmount;
                })
                .sum();
        double refundedInvoices = validInvoices
                .stream()
                .filter(i -> i.getPaymentStatus().equalsIgnoreCase(PaymentStatus.REFUNDED.name()) || i.getPaymentStatus().equalsIgnoreCase(PaymentStatus.PARTIAL_REFUND.name()))
                .mapToDouble(i -> {
                    if (i.getPaidAmount() == null) {
                        return 0.0;
                    }
    ...
    public int recordPayment(String invoiceId, String status, double amount) {
        ...
        if (invoice != null) {
            ...
            if (invoice.getInvoiceType().equalsIgnoreCase(InvoiceType.ADVANCE.name()) || invoice.getInvoiceType().equalsIgnoreCase(InvoiceType.ADDITIONAL_ADVANCE.name())) {
                ...
                if (deductionAmount > 0 && (paidAmount + amount) < deductionAmount) {
                    ...
                    if (listDeductions != null) {
                        ...
                        List<Deductions> newDeductions = listDeductions.stream().map(i -> {
                    ...
                    if (paidAfterDeduction > 0) {
                        ...
                        double amountToCredit = paidAfterDeduction - redeemedAmount;
                        invoice.setBalanceAmount(amountToCredit);
...
//                        }
                    ...
                    if (deductionAmount > 0) {
                        if (invoice.getDeductions() != null) {
                            double pendingAmount = invoice.getDeductions().stream().filter(i -> {
                                if (i.getPaidAmount() != null) {
                                    return !i.getPaidAmount().equals(i.getAmount());
                                }
                                return false;
                            }).mapToDouble(i1 -> i1.getAmount() - i1.getPaidAmount()).sum();
                            if (pendingAmount > 0) {
                                double pAmount = 0.0;
                                if (invoice.getPaidAmount() != null) {
...
//                                        }
    ...
    public Double getBookingAmount(String customerId, String hostelId) {
        ...
        if (invoiceV1 != null) {
            if (invoiceV1.getPaidAmount() != null) {
    ...
    public List<InvoicesV1> getAllAdvanceInvoiceDetails(String customerId, String hostelId) {
        List<String> invoiceTypes = new ArrayList<>();
        invoiceTypes.add(InvoiceType.ADDITIONAL_ADVANCE.name());
        invoiceTypes.add(InvoiceType.ADVANCE.name());
        List<InvoicesV1> listInvoices = invoicesV1Repository.findByCustomerIdAndHostelIdAndInvoiceType(customerId, hostelId, invoiceTypes);
    ...
    public ResponseEntity<?> generateManualInvoice(String customerId, ManualInvoice manualInvoice) {
        ...
        if (currentBillingDate.billingModel().equalsIgnoreCase(BillingModel.POSTPAID.name())) {
            if (Utils.compareWithTwoDates(invoiceDate, currentBillingDate.currentBillStartDate()) >= 0) {
                return new ResponseEntity<>(Utils.CANNOT_CREATE_CURRENT_MONTH_INVOICE_POSTPAID, HttpStatus.BAD_REQUEST);
            }
        }
...
//        Date dateEndDate = Utils.findLastDate(day, cal.getTime());
        ...
        for (ItemResponse item : manualInvoice.items()) {
            ...
            if (itemName.equalsIgnoreCase("EB")) {
                invoiceItem.setAmount(Utils.roundOffWithTwoDigit(item.amount()));
                ...
                invoiceItem.setAmount(Utils.roundOffWithTwoDigit(invoiceAmount));
        ...
        invoicesV1.setDeductions(null);
        invoicesV1.setDeductionAmount(0.0);
        invoicesV1.setSubTotal(Utils.roundOfDouble(totalAmount));
...
//        invoicesV1.setEbAmount(ebAmount);
    ...
    private ResponseEntity<?> generateManualInvoiceForJoiningBasedTenants(String hostelId, Customers customers, ManualInvoice manualInvoice, Users users) {
        BookingsV1 bookingsV1 = bookingsService.getBookingsByCustomerId(customers.getCustomerId());
        BillingDates currentBillStartAndEndDates = hostelService.getJoiningBasedCurrentMonthBillingDate(customers.getJoiningDate(), hostelId, new Date());

        Date requestedInvoiceDate = Utils.stringToDate(manualInvoice.invoiceDate().replaceAll("/", "-"), Utils.USER_INPUT_DATE_FORMAT);
        BillingDates billingDatesForRequestedDate = hostelService.getJoiningBasedCurrentMonthBillingDate(customers.getJoiningDate(), hostelId, requestedInvoiceDate);

        if (Utils.compareWithTwoDates(billingDatesForRequestedDate.currentBillStartDate(), bookingsV1.getJoiningDate()) < 0) {
            System.out.println(Utils.CUSTOMER_NOT_CHECKED_IN_DATE);
            return new ResponseEntity<>(Utils.CUSTOMER_NOT_CHECKED_IN_DATE, HttpStatus.BAD_REQUEST);
        }

        List<String> invoiceTypes = new ArrayList<>();
        invoiceTypes.add(InvoiceType.RENT.name());
        List<InvoicesV1> invoicesForSelectedDate = invoicesV1Repository.findInvoicesByCustomerIdAndTypeInAndDate(customers.getCustomerId(), invoiceTypes, billingDatesForRequestedDate.currentBillStartDate(), billingDatesForRequestedDate.currentBillEndDate());

        if (!invoicesForSelectedDate.isEmpty()) {
            return new ResponseEntity<>(Utils.INVOICE_ALREADY_PRESENT, HttpStatus.BAD_REQUEST);
        }
        ...
        if (Utils.checkNullOrEmpty(manualInvoice.invoiceNumber())) {
            ...
            InvoicesV1 inv = invoicesV1Repository.findLatestInvoiceByPrefix(prefix, hostelId);
            if (inv != null) {
                String[] prefArr = inv.getInvoiceNumber().split("-");
                if (prefArr.length > 1) {
                    int suffix = Integer.parseInt(prefArr[1]) + 1;
        ...
        if (Utils.compareWithTwoDates(requestedInvoiceDate, new Date()) > 0) {
            return new ResponseEntity<>(Utils.FUTURE_DATES_NOT_ALLOWED, HttpStatus.BAD_REQUEST);
        }

        BookingsV1 filteredBooking = bookingsService.getBookingByCustomerIdAndDate(customers.getCustomerId(), billingDatesForRequestedDate.currentBillStartDate(), billingDatesForRequestedDate.currentBillEndDate());
        if (filteredBooking == null) {
            return new ResponseEntity<>(Utils.CUSTOMER_NOT_CHECKED_IN_DATE, HttpStatus.BAD_REQUEST);
        }

        List<InvoicesV1> previousInvoices = invoicesV1Repository.findInvoiceByCustomerIdAndDate(customers.getCustomerId(), billingDatesForRequestedDate.currentBillStartDate(), billingDatesForRequestedDate.currentBillEndDate());
        if (previousInvoices != null) {
            if (!previousInvoices.isEmpty()) {
                return new ResponseEntity<>(Utils.INVOICE_ALREADY_PRESENT, HttpStatus.BAD_REQUEST);
            }
        ...
        boolean isCurrentCycle = Utils.compareWithTwoDates(currentBillStartAndEndDates.currentBillStartDate(), billingDatesForRequestedDate.currentBillStartDate()) <= 0;
        ...
        for (ItemResponse item : manualInvoice.items()) {
            ...
            if (itemName.equalsIgnoreCase("EB")) {
                ...
                invoiceItem.setAmount(rentAmount);
                invoiceItem.setInvoiceItem(com.smartstay.smartstay.ennum.InvoiceItems.RENT.name());
            } else if (itemName.equalsIgnoreCase("AMENITY")) {
                invoiceItem.setAmount(item.amount());
                invoiceItem.setInvoiceItem(com.smartstay.smartstay.ennum.InvoiceItems.AMENITY.name());
            ...
            if (item.amount() != null) {
                if (itemName.equalsIgnoreCase("RENT") || itemName.equalsIgnoreCase("ROOM RENT")) {
                    totalAmount = totalAmount + rentAmount;
                } else {
                    totalAmount = totalAmount + item.amount();
                }
            }
...
//        invoicesV1.setEbAmount(ebAmount);
        ...
        invoicesV1.setCreatedBy(authentication.getName());
        invoicesV1.setInvoiceGeneratedDate(new Date());
        invoicesV1.setInvoiceDate(billingDatesForRequestedDate.currentBillStartDate());
        invoicesV1.setInvoiceStartDate(Utils.convertToTimeStamp(billingDatesForRequestedDate.currentBillStartDate()));
        invoicesV1.setInvoiceDueDate(Utils.convertToTimeStamp(billingDatesForRequestedDate.dueDate()));
        invoicesV1.setInvoiceEndDate(Utils.convertToTimeStamp(billingDatesForRequestedDate.currentBillEndDate()));
        invoicesV1.setCancelled(false);

        invoicesV1.setInvoiceItems(listInvoicesItems);

        InvoicesV1 invoicesV11 = invoicesV1Repository.save(invoicesV1);
        usersService.addUserLog(hostelId, invoicesV11.getInvoiceId(), ActivitySource.INVOICE, ActivitySourceType.MANUAL_BILL, users);
        return new ResponseEntity<>(Utils.CREATED, HttpStatus.CREATED);
    }
    ...
    public ResponseEntity<?> getInvoiceDetailsByInvoiceId(String hostelId, String invoiceId) {
        ...
        boolean canApplyToOtherInvoice = false;
        ...
        List<InvoicesV1> advanceInvoices = invoicesV1Repository.findAdditionalAdvanceInvoiceByCustomerId(customers.getCustomerId());
        if (advanceInvoices != null) {
            boolean canApply = advanceInvoices.stream()
                    .anyMatch(i ->
                            (i.getPaymentStatus().equalsIgnoreCase(PaymentStatus.PAID.name())
                                    || i.getPaymentStatus().equalsIgnoreCase(PaymentStatus.PARTIAL_PAYMENT.name()))
                                    && i.getBalanceAmount() != null
                                    && i.getBalanceAmount() > 0
                    );

            canApplyToOtherInvoice = canApply;
            InvoicesV1 selectedAdvanceInvoice = advanceInvoices
                    .stream()
                    .filter(i -> i.getInvoiceId().equalsIgnoreCase(invoicesV1.getInvoiceId()))
                    .findFirst()
                    .orElse(null);

            if (selectedAdvanceInvoice == null) {
                canApplyToOtherInvoice = false;
                if (canApply) {
                    isAdvanceAvailableForRedeem = true;
                    availableAdvanceAmount = advanceInvoices
                            .stream()
                            .mapToDouble(i -> {
                                if (i.getPaymentStatus().equalsIgnoreCase(PaymentStatus.PAID.name()) || i.getPaymentStatus().equalsIgnoreCase(PaymentStatus.PARTIAL_PAYMENT.name())) {
                                    if (i.getBalanceAmount() != null && i.getBalanceAmount() > 0) {
...
//                }
        ...
        if (canApplyToOtherInvoice) {
            List<InvoicesV1> unpaidRentInvoices = findUnpaidInvoices(customers.getCustomerId()).stream().filter(i -> i.getInvoiceType().equalsIgnoreCase(InvoiceType.RENT.name()) || i.getInvoiceType().equalsIgnoreCase(InvoiceType.REASSIGN_RENT.name())).toList();
        ...
        if (invoicesV1.getInvoiceType().equalsIgnoreCase(InvoiceType.SETTLEMENT.name())) {
            ...
            Double total1 = 0.0;
            Double total2 = 0.0;
            if (invoicesV1.getBasePrice() != null) {
                total1 = invoicesV1.getBasePrice() - totalDeductionAmount;
            ...
            InvoiceInfo invoiceInfo = new InvoiceInfo(invoicesV1.getInvoiceId(),
                    ...
                    invoiceMonth.toString(), paymentStatus, invoicesV1.getInvoiceMode(),
...
//        paidAmount = transactionService.findPaidAmountForInvoice(invoiceId);
        ...
        if (invoicesV1.getBasePrice() != null) {
            total1 = invoicesV1.getBasePrice() - deductionAmount;

            if (invoicesV1.getInvoiceType().equalsIgnoreCase(InvoiceType.ADVANCE.name())) {
                total1 = invoicesV1.getSubTotal() - deductionAmount;
            }
        }
        ...
        if (invoicesV1.getInvoiceType().equalsIgnoreCase(InvoiceType.BOOKING.name())) {
            ...
            boolean isCustomerCheckedOut = customers != null && customers.getCurrentStatus() != null && customers.getCurrentStatus().equalsIgnoreCase(CustomerStatus.VACATED.name());
            ...
            invoiceInfo = new InvoiceInfo(invoicesV1.getInvoiceId(),
                    ...
                    invoiceRentalPeriod.toString(), invoiceMonth.toString(),
                    paymentStatus,
                    invoicesV1.getInvoiceMode(),
                    invoicesV1.getInvoiceType(),
                    null,
                    null,
                    null,
            ...
            if (invoicesV1.getInvoiceType().equalsIgnoreCase(InvoiceType.EB_HOLDING.name()) ||
                ...
                paidBy = retainerRelationService.getRelationDetails(invoicesV1.getInvoiceId());
                InvoiceNotes invoiceNotes = invoiceNotesService.getInvoiceNotes(invoicesV1.getInvoiceId());
                if (invoiceNotes != null) {
                    description = invoiceNotes.description();
                    detailedDescription = invoiceNotes.detailedDescription();
                }
            }
        ...
        InvoiceDetails details = new InvoiceDetails(invoicesV1.getInvoiceNumber(), invoicesV1.getInvoiceId(), invoiceDate, Utils.dateToString(invoicesV1.getInvoiceDueDate()), hostelEmail, hostelPhone, "91", customers.getHostelId(), customerInfo, stayInfo, invoiceInfo, accountDetails, paymentHistoryList, signatureInfo);
    ...
    private ResponseEntity<?> getSettlementInvoiceDetails(SettlementItems settlementItems, ConfigInfo signatureInfo, StayInfo stayInfo, AccountDetails accountDetails, CustomerInfo customerInfo, InvoicesV1 invoicesV1, HostelV1 hostelV1) {
        ...
        if (settlementItems.getUnpaidInvoices() != null) {
            if (!settlementItems.getUnpaidInvoices().isEmpty()) {
                ...
                double unpaidInvoiceTotalAmount = settlementItems.getUnpaidInvoices().stream().mapToDouble(i -> {
                ...
                List<UnpaidInvoiceItem> listUnpaidInvoiceItems = settlementItems.getUnpaidInvoices().stream().map(i -> {
                ...
                unpaidInvoiceInfo = new UnpaidInvoiceInfo(noOfInvoiceInfo, Utils.roundOffWithTwoDigit(unpaidInvoiceTotalAmount), listUnpaidInvoiceItems);
            }
        }
        ...
        if (invoicesV1.getDeductions() != null) {
            if (!invoicesV1.getDeductions().isEmpty()) {
                double totalDeductionAmount = invoicesV1.getDeductions().stream().mapToDouble(i -> {
                    if (i.getAmount() != null) {
                        return i.getAmount();
                    }
                    return 0.0;
                ...
                double paidAmount = invoicesV1.getDeductions().stream().mapToDouble(i -> {
                    if (i.getPaidAmount() != null) {
                        return i.getPaidAmount();
                    }
                    return 0.0;
                ...
                List<DeductionsItem> listDeductionItems = invoicesV1.getDeductions().stream().map(i -> {
                    double pAmount = 0.0;
                    if (i.getPaidAmount() != null) {
                ...
                deductionsInfo = new DeductionsInfo(Utils.roundOffWithTwoDigit(totalDeductionAmount), Utils.roundOffWithTwoDigit(paidAmount), Utils.roundOffWithTwoDigit(pendingAmount), listDeductionItems);
            }
        }
...
//                subTotal = subTotal - bookingItems.availableAdvanceBalance();
        ...
        if (settlementItems.getCurrentRentBreakUps() != null) {
            if (!settlementItems.getCurrentRentBreakUps().isEmpty()) {
                com.smartstay.smartstay.dto.settlement.CurrentRentBreakUp latestRent = settlementItems.getCurrentRentBreakUps().stream().max(Comparator.comparing(com.smartstay.smartstay.dto.settlement.CurrentRentBreakUp::getFromDate)).orElse(null);

                listRentBreakUp = settlementItems.getCurrentRentBreakUps().stream().map(i -> new CurrentRentBreakUp(latestRent, isFullRentSelected, fullRentAmount).apply(i)).toList();
            }
        }
...
//            subTotal = subTotal - currentPaidAmount;
        ...
        if (listRentBreakUp != null) {
            if (!listRentBreakUp.isEmpty()) {
                stayDays = listRentBreakUp.stream().mapToInt(i -> (int) i.noOfDays()).sum();
            }
        }

        if (settlementItems.getCurrentMonthOtherItems() != null) {
            currentMonthOtherAmounts = settlementItems.getCurrentMonthOtherItems().stream().mapToDouble(i -> {
                if (i.getAmount() != null) {
                    return i.getAmount();
                }
                return 0.0;
        ...
        if (settlementItems.getCurrentMonthOtherItems() != null) {
            if (!settlementItems.getCurrentMonthOtherItems().isEmpty()) {
                listCurrentMonthOtherItems = settlementItems.getCurrentMonthOtherItems().stream().map(i -> new CurrentMonthOtherItems(i.getOtherItem(), i.getAmount())).toList();
            }
        }

        if (settlementItems.getEbItems() != null) {
            if (!settlementItems.getEbItems().isEmpty()) {
                List<EBItems> ebItemsList = settlementItems.getEbItems().stream().map(i -> {
                ...
                double ebTotalAmount = settlementItems.getEbItems().stream().mapToDouble(i -> {
                    if (i.getTotalAmount() == null) {
                        return 0.0;
                    }
        ...
        if (settlementItems.getWalltetItems() != null) {
            ...
            walletAmount = listWalletItems.stream().mapToDouble(i -> {
                if (i.getAmount() != null) {
                    return i.getAmount();
                }
                return 0.0;
            }).sum();
            List<WalletItems> listwalletItems = listWalletItems.stream().map(i -> new WalletItems(i.getType(), i.getType(), Utils.roundOffWithTwoDigit(i.getAmount()))).toList();
        ...
        currentRentInfo = new CurrentRentInfo(currentMonthLabelText, Utils.roundOffWithTwoDigit(currentPaidAmount), Utils.roundOffWithTwoDigit(currentPayablemount), Utils.roundOffWithTwoDigit(stayDays), Utils.roundOffWithTwoDigit(currentMonthPayableRent), Utils.roundOffWithTwoDigit(currentMonthOtherAmounts), listRentBreakUp, listCurrentMonthOtherItems);
        RetainerInfo retainerInfo = null;
        if (settlementItems.getRetainerItems() != null) {
            List<RetainerItems> listRetainerItems = settlementItems.getRetainerItems();
            int noOfRetaineApplied = listRetainerItems.size();
            ...
            retainerInfo = new RetainerInfo(noOfRetaineApplied,
                    totalRetainerApplied,
                    items);
        }

        HeaderInfo headerInfo = null;
        if (hostelV1 != null) {
            headerInfo = new HeaderInfo(invoicesV1.getInvoiceNumber(), hostelV1.getHouseNo(), hostelV1.getStreet(), hostelV1.getCity(), hostelV1.getState(), hostelV1.getPincode(), null, hostelV1.getMobile(), "91", hostelV1.getMainImage(), hostelV1.getEmailId());
        }
...
//       totalPayable = totalPayable + unpaidInvoiceAmount + deductionAmount;
    ...
    public List<InvoiceResponse> getInvoiceResponseList(String customerId, List<String> invoiceTypes) {
        List<InvoicesV1> invoices = invoicesV1Repository.findNonRetainerInvoiceByCustomerId(customerId, invoiceTypes);
...
//            settlementInvoice.setNewCancelledInvoices(listNewCancelledInvoices);
            ...
            double deductionAmount = settlementDeductions.stream().mapToDouble(Deductions::getAmount).sum();

            settlementInvoice.setDeductionAmount(deductionAmount);

//            List<InvoiceItems> listInvoiceItems = listDeductions.stream().map(i -> {
//                InvoiceItems invoiceItems = new InvoiceItems();
//                invoiceItems.setAmount(i.getAmount());
//                deductionAmount = deductionAmount + i.getAmount();
...
//            List<InvoicesV1> listInvoices = invoicesV1Repository.findInvoiceByCustomerIdAndDate(customers.getCustomerId(), billingDates.currentBillStartDate(), billingDates.currentBillEndDate());
            ...
            if (latestInvoice != null) {
                ...
                if (Utils.compareWithTwoDates(dateJoiningDate, latestInvoice.getInvoiceStartDate()) == 0) {
                    ...
                    latestInvoice.setSubTotal(totalAmountUpdate);
                    ...
                    latestInvoice.setSubTotal(totalAmountForOldInvoice);
    ...
    private ReassignRent calculateAndCreateNewInvoiceForJoiningBasedHostels(Customers customers, Date joiningDate, Double newRent, String strJoiningDate) {
        BillingDates joiningBasedBillingRule = hostelService.getJoiningBasedCurrentMonthBillingDate(customers.getJoiningDate(), customers.getHostelId(), joiningDate);
        if (joiningBasedBillingRule != null) {
            InvoicesV1 latestInvoice = invoicesV1Repository.findCurrentRunningInvoice(customers.getCustomerId(), joiningBasedBillingRule.currentBillStartDate());
            if (latestInvoice != null) {
...
//                double rent = latestInvoice.getInvoiceItems().stream().filter(item -> com.smartstay.smartstay.ennum.InvoiceItems.RENT.name().equalsIgnoreCase(item.getInvoiceItem())).mapToDouble(InvoiceItems::getAmount).sum();
                double rent = bookingsService.getCurrentRent(customers.getCustomerId());

                if (Utils.compareWithTwoDates(joiningDate, latestInvoice.getInvoiceStartDate()) == 0) {

                    double rentForNewInvoice = newRent;
                    ...
                    Calendar lastDayCal = Calendar.getInstance();
                    lastDayCal.setTime(joiningDate);
                    lastDayCal.add(Calendar.DAY_OF_MONTH, -1);

                    long noOfDaysInOldInvoice = Utils.findNumberOfDays(startDate, lastDayCal.getTime());
                    latestInvoice.setInvoiceEndDate(lastDayCal.getTime());

                    double rentForOldInvoice = noOfDaysInOldInvoice * rentPerday;
                    double totalAmountForOldInvoice = rentForOldInvoice;
                    ...
                    return createNewInvoice(latestInvoice, strJoiningDate, newRent, joiningBasedBillingRule, balanceAmount);
                }
    ...
    public ReassignRent createNewInvoice(InvoicesV1 oldInvoice, String joiningDate, Double rent, BillingDates billingDates, double balanceAmount) {
        ...
        invoicesV1.setSubTotal(rentForNewInvoice);
        ...
        invoicesV1.setGstPercentile(0.0);
        invoicesV1.setCreatedAt(new Date());
        invoicesV1.setInvoiceDate(dateJoiningDate);
        invoicesV1.setInvoiceGeneratedDate(new Date());
        invoicesV1.setCancelled(false);
        invoicesV1.setInvoiceMode(InvoiceMode.AUTOMATIC.name());
...
//        }
    ...
    public void updateJoiningDateOnAdvanceInvoice1(String customerId, Date joiningDate) {
        List<InvoicesV1> invoicesV1 = invoicesV1Repository.findByCustomerIdAndInvoiceTypeIn(customerId, List.of(InvoiceType.ADVANCE.name(), InvoiceType.RENT.name()));

        if (invoicesV1 != null && !invoicesV1.isEmpty()) {
            BillingDates billingDates = hostelService.getBillingRuleOnDate(invoicesV1.getFirst().getHostelId(), joiningDate);
    ...
    public void findOldInvoiceAndUpdate(String customerId, Date oldJoiningDate, Date newJoiningDate, String hostelId, Double rent) {
        ...
        if (oldInvoices != null && !oldInvoices.isEmpty()) {
            ...
            if (Utils.compareWithTwoDates(newJoiningDate, billingDates.currentBillStartDate()) < 0) {
                ...
                invoicesV1.setSubTotal(oldBasePrice + newRent);
                invoicesV1.setDeductionAmount(0.0);
...
//                invoicesV1Repository.save(newJoiningDateInvoice);
                ...
                invoicesV1.setSubTotal(newRent);
                invoicesV1.setTotalAmount(0.0);
...
//                invoicesV1.setInvoiceItems(invoiceItems);
    ...
    public boolean updateJoiningDate(Customers customers, Date joinigDate, String hostelId, Date oldJoiningDate, Date newJoiningDate, Double rent) {
        ...
        if (currentMonthBillingDates.typeOfBilling().equalsIgnoreCase(BillingType.JOINING_DATE_BASED.name())) {
            ...
            List<InvoicesV1> findPaidOrParialyPaidInvoices = listInvoices.stream().filter(i -> i.getPaymentStatus().equalsIgnoreCase(PaymentStatus.PAID.name()) || i.getPaymentStatus().equalsIgnoreCase(PaymentStatus.PARTIAL_PAYMENT.name())).toList();
            if (!findPaidOrParialyPaidInvoices.isEmpty()) {
                return false;
            }
            if (Utils.compareWithTwoDates(oldJoiningDate, currentMonthBillingDates.currentBillStartDate()) < 0) {
                ...
                //old joining date is less than current month
                if (Utils.compareWithTwoDates(oldJoiningDate, currentMonthBillingDates.currentBillStartDate()) >= 0) {
                    //checking new joining date is less than month start
                    if (Utils.compareWithTwoDates(newJoiningDate, currentMonthBillingDates.currentBillStartDate()) < 0) {
                        findAndDeleteRentalInvoice(customers.getCustomerId(), oldJoiningDate);
                    } else {
                        //current month to current month
                        findAndUpdateRentalInvoice(customers.getCustomerId(), oldJoiningDate, joinigDate, currentMonthBillingDates, rent);
                    }
            ...
            if (currentMonthBillingDates.billingModel().equalsIgnoreCase(BillingModel.PREPAID.name())) {
                ...
                List<InvoicesV1> listCurrentMonthOtherInvoices = invoicesV1Repository.findCurrentMonthOtherInvoices(customers.getCustomerId(), customers.getHostelId(), currentMonthBillingDates.currentBillStartDate());
                if (listCurrentMonthOtherInvoices != null && !listCurrentMonthOtherInvoices.isEmpty()) {
                    return false;
                }
                //old to old
                //old joining date was older than current month
                if (Utils.compareWithTwoDates(oldJoiningDate, currentMonthBillingDates.currentBillStartDate()) < 0) {
//                    updates date from old date to old date
                    if (Utils.compareWithTwoDates(newJoiningDate, currentMonthBillingDates.currentBillStartDate()) <= 0) {
...
//                        findOldInvoiceAndUpdate(customers.getCustomerId(), oldJoiningDate, joinigDate, hostelId, rent);
    ...
    private boolean calculateAndCreateNewInvoiceForPostpaid(Double rent, Customers customers, Date newJoiningDate, InvoicesV1 currentRunningInvoice, BillingDates runningInvoiceDates) {
        ...
        invoiceDueDate = Utils.addDaysToDate(invoiceDate, runningInvoiceDates.dueDays() - 1);
    ...
    private boolean createFullRentRunningInvoice(Customers customers, Double fullrent, InvoicesV1 currentRunningInvoice, BillingDates runningInvoiceDates) {
        if (currentRunningInvoice == null) {
            ...
            Date dueDate = Utils.addDaysToDate(calendar.getTime(), runningInvoiceDates.dueDays() - 1);
            ...
            currentRunningInvoice.setGstPercentile(0.0);
            currentRunningInvoice.setInvoiceGeneratedDate(new Date());
            currentRunningInvoice.setInvoiceDate(Utils.convertToTimeStamp(calendar.getTime()));
            currentRunningInvoice.setInvoiceStartDate(Utils.convertToTimeStamp(invoiceStartDate));
            currentRunningInvoice.setInvoiceDueDate(Utils.convertToTimeStamp(dueDate));
            currentRunningInvoice.setInvoiceEndDate(Utils.convertToTimeStamp(runningInvoiceDates.currentBillEndDate()));
    ...
    private void findAndUpdateCurrentMonthInvoiceDates(String customerId, String hostelId, Double rent, BillingDates currentMonthBillingDates, Date newJoiningDate) {
        ...
        if (currentMonthInvoice != null) {
            ...
            if (Utils.compareWithTwoDates(newJoiningDate, currentMonthBillingDates.currentBillStartDate()) <= 0) {
                ...
                long noOfDaysInCurrentMonth = Utils.findNumberOfDays(currentMonthBillingDates.currentBillStartDate(), currentMonthBillingDates.currentBillEndDate());
                long daysStayingInCurrentMonth = Utils.findNumberOfDays(newJoiningDate, currentMonthBillingDates.currentBillEndDate());
                double rentPerDay = rent / noOfDaysInCurrentMonth;
    ...
    private void findAndDeleteRentalInvoice(String customerId, Date oldJoiningDate) {
        ...
        Date endDate = Utils.findLastDate(Utils.dateToDate(oldJoiningDate), oldJoiningDate);
        List<InvoicesV1> listInvoices = invoicesV1Repository.findInvoicesByCustomerIdAndTypeInAndDate(customerId, invoiceTypes, oldJoiningDate, endDate);
        if (listInvoices.size() == 1) {
            invoicesV1Repository.deleteAll(listInvoices);
        }
    }

    private void findAndUpdateAdvanceInvoice(String customerId, String hostelId, Date joiningDate, BillingDates currentMonthBillingDates) {
        List<String> advanceType = new ArrayList<>();
        advanceType.add(InvoiceType.ADVANCE.name());
        List<InvoicesV1> advanceInvoice = invoicesV1Repository.findByCustomerIdAndInvoiceTypeIn(customerId, advanceType);
        if (advanceInvoice.size() == 1) {
            int join = Utils.dateToDate(joiningDate);
            Date endDate = Utils.findLastDate(join, joiningDate);
            Date dueDate = Utils.addDaysToDate(joiningDate, currentMonthBillingDates.dueDays() - 1);
            List<InvoicesV1> newAdvanceInvoice = advanceInvoice.stream().map(i -> {
                i.setInvoiceStartDate(joiningDate);
                i.setInvoiceDueDate(dueDate);
                i.setInvoiceEndDate(endDate);
                return i;
            }).toList();
            invoicesV1Repository.saveAll(newAdvanceInvoice);

        }
    }

    private void createNewInvoiceForCurrentMonthForJoiningBasedBilling(Customers customers, String hostelId, Date joinigDate, Double rent, BillingDates billingDates) {
        InvoicesV1 invoicesV1 = new InvoicesV1();

        Calendar currentMonthJoining = Calendar.getInstance();
        currentMonthJoining.set(Calendar.DAY_OF_MONTH, Utils.dateToDate(joinigDate));
...
//        Date joiningDate1 = Utils.stringToDate(joiningDate.replace("/", "-"), Utils.USER_INPUT_DATE_FORMAT);
        Date dueDate = Utils.addDaysToDate(newDateForCurrentMonth, billingDates.dueDays() - 1);
        Date endDate = Utils.findLastDate(Utils.dateToDate(newDateForCurrentMonth), newDateForCurrentMonth);

        invoicesV1.setTotalAmount(rent);
        invoicesV1.setBasePrice(rent);
        invoicesV1.setSubTotal(rent);
        invoicesV1.setDeductionAmount(0.0);
        ...
        invoicesV1.setCreatedAt(new Date());
        invoicesV1.setInvoiceDate(newDateForCurrentMonth);
    ...
    private void findAndUpdateRentalInvoice(String customerId, Date oldJoiningDate, Date newJoiningDate, BillingDates billingDates, Double rent) {
        Date lastDateAsPerOldJoining = Utils.findLastDate(Utils.dateToDate(oldJoiningDate), oldJoiningDate);
        List<String> invoiceTypes = new ArrayList<>();
        invoiceTypes.add(InvoiceType.RENT.name());
        List<InvoicesV1> listInvoices = invoicesV1Repository.findInvoicesByCustomerIdAndTypeInAndDate(customerId, invoiceTypes, oldJoiningDate, lastDateAsPerOldJoining);
        if (listInvoices.size() == 1) {
            Date invoiceEndDate = Utils.findLastDate(Utils.dateToDate(newJoiningDate), newJoiningDate);
            Date dueDate = Utils.addDaysToDate(newJoiningDate, billingDates.dueDays() - 1);
            InvoicesV1 invoicesV1 = listInvoices.get(0);
            invoicesV1.setInvoiceStartDate(newJoiningDate);
    ...
    private void createNewInvoiceAfterForCurrentBillingCycle(Customers customer, Date joinigDate, String hostelId, Double rent, BillingDates billingDates) {
        ...
        InvoicesV1 invoicesV1 = new InvoicesV1();
        invoicesV1.setSubTotal(totalRentForCurrentMonth);
        invoicesV1.setDeductionAmount(0.0);
        ...
        invoicesV1.setCreatedBy(authentication.getName());
        invoicesV1.setCreatedAt(new Date());
        invoicesV1.setInvoiceDate(joinigDate);
        invoicesV1.setInvoiceType(InvoiceType.RENT.name());
...
//        invoiceItemService.updateInvoiceItems(invoiceItems);
    ...
    public void updateAdvaceAmount(InvoicesV1 advanceInvoice, Double newAdvance) {
        ...
        if (advanceInvoice.getDeductionAmount() != null) {
            deductionAmount = advanceInvoice.getDeductionAmount();
        }
        advanceInvoice.setTotalAmount(newAdvance + deductionAmount);
        advanceInvoice.setSubTotal(newAdvance + deductionAmount);
        ...
        if (invoicesV1.getInvoiceType().equalsIgnoreCase(InvoiceType.ADVANCE.name())) {
            ...
            if (deductionAmount != null && deductionAmount > 0) {
                if (invoicesV1.getPaidAmount() < deductionAmount) {
                    ...
                    List<Deductions> listDeductions = invoicesV1.getDeductions().stream().map(i -> {
...
//        }
    ...
    private Map<String, List<InvoiceRefundHistory>> getFinalSettlementHistoryList(InvoicesV1 settlementInvoice, List<InvoiceSummary> relatedInvoices123) {
...
//        invoiceIds.add(settlementInvoice.getInvoiceId());
        ...
        Map<String, TransactionV1> transactionMap = transactions.stream().collect(Collectors.toMap(TransactionV1::getInvoiceId, t -> t, (t1, t2) -> t1));

        Set<String> bankIds = transactions.stream().map(TransactionV1::getBankId).filter(Objects::nonNull).collect(Collectors.toSet());
        ...
        if (transactions != null) {
            transactions.forEach(item -> {
                ...
                if (item != null && item.getBankId() != null) {
                    ...
                    if (bank != null) {
                        paymentMode = (bank.getAccountHolderName() != null ? bank.getAccountHolderName() : "") + "-" + (bank.getAccountType() != null ? bank.getAccountType() : "");
                    }
                }

                if (settlementInvoice.getPaymentStatus() != null) {
                    if (settlementInvoice.getPaymentStatus().equalsIgnoreCase(PaymentStatus.REFUNDED.name()) || settlementInvoice.getPaymentStatus().equalsIgnoreCase(PaymentStatus.PARTIAL_REFUND.name())) {
                        refundHistory.add(new InvoiceRefundHistory(item.getTransactionReferenceId(), Utils.dateToString(item.getPaymentDate()), Utils.dateToTime(item.getPaymentDate()), paymentMode, item.getPaidAmount() != null ? item.getPaidAmount() : 0.0, paymentMode, paymentMode));
                    } else {
                        paymentHistory.add(new InvoiceRefundHistory(item.getTransactionReferenceId(), Utils.dateToString(item.getPaymentDate()), Utils.dateToTime(item.getPaymentDate()), paymentMode, item.getPaidAmount() != null ? item.getPaidAmount() : 0.0, paymentMode, paymentMode));
                    }
                }
    ...
    public ResponseEntity<?> markInvoiceUnpaid(String hostelId, String invoiceId) {
        ...
        if (invoicesV1.isDiscounted()) {
            return new ResponseEntity<>(Utils.CANNOT_MARK_UNPAID_DISCOUNT_APPLIED, HttpStatus.BAD_REQUEST);
        }
        if (!invoicesV1.getHostelId().equalsIgnoreCase(hostelId)) {
            return new ResponseEntity<>(Utils.INVALID_REQUEST, HttpStatus.BAD_REQUEST);
        }
    ...
    public ResponseEntity<?> applyinvoiceDiscount(String hostelId, String invoiceId, ApplyDiscount discount) {
        ...
        if (discount.discountPercentage() != null) {
            ...
            discountAmount = Utils.roundOfDouble((percentage / 100) * invoicesV1.getBasePrice());
...
//        Date joiningDate1 = Utils.stringToDate(joiningDate.replace("/", "-"), Utils.USER_INPUT_DATE_FORMAT);
    ...
    public double invoicesPaidAmountByType(String customerId, String name) {
        ...
        if (listInvoices != null) {
            paidAmount = listInvoices.stream().mapToDouble(i -> {
                if (i.getPaymentStatus().equalsIgnoreCase(PaymentStatus.PAID.name()) || i.getPaymentStatus().equalsIgnoreCase(PaymentStatus.PARTIAL_PAYMENT.name())) {
                    if (i.getPaidAmount() != null) {
                        return i.getPaidAmount();
                    }
    ...
    public List<UnpaidInvoices> getUnpaidInvoices(String customerId, String hostelId, Date leavingDate) {
        ...
        invoicesTypes.add(InvoiceType.RENT.name());
        invoicesTypes.add(InvoiceType.REASSIGN_RENT.name());
        List<InvoicesV1> listOldInvoices = invoicesV1Repository.findOldMonthPendingInvoices(customerId, billingDates.currentBillStartDate(), invoicesTypes);

        return listOldInvoices.stream().map(i -> new UnpaidInvoicesMapper().apply(i)).toList();
    }

    /**
     * this is for generating current month info
     *
     * @param hostelId
     * @param customers
     * @param leavingDate
    ...
    public RentInfo getRentInfoForSettlement(String hostelId, Customers customers, Date leavingDate, double monthlyRent) {
        ...
        List<String> discountedInvoiceIds = new ArrayList<>();

        Date currentMonthStartDate = null;
        if (Utils.compareWithTwoDates(customers.getJoiningDate(), billingDates.currentBillStartDate()) < 0) {
            currentMonthStartDate = billingDates.currentBillStartDate();
        } else {
            currentMonthStartDate = customers.getJoiningDate();
        }
        AtomicBoolean isDiscountApplied = new AtomicBoolean(false);
        double discountAmount = 0.0;
        ...
        listInvoices.forEach(item -> {
            if (item.isDiscounted()) {
                isDiscountApplied.set(true);
                discountedInvoiceIds.add(item.getInvoiceId());
            }
            otherItems.addAll(item.getInvoiceItems().stream().filter(i -> !i.getInvoiceItem().equalsIgnoreCase(com.smartstay.smartstay.ennum.InvoiceItems.RENT.name())).map(i -> {
                if (i.getAmount() != null) {
                    otherItemAmount.set(otherItemAmount.get() + i.getAmount());
                }
                String itemName = null;
                if (i.getInvoiceItem().equalsIgnoreCase(com.smartstay.smartstay.ennum.InvoiceItems.OTHERS.name())) {
                    itemName = i.getOtherItem();
                } else {
                    if (i.getInvoiceItem().equalsIgnoreCase(com.smartstay.smartstay.ennum.InvoiceItems.EB.name())) {
        ...
        if (runningInvoice != null) {
            if (runningInvoice.isDiscounted()) {
                isDiscountApplied.set(true);
                discountedInvoiceIds.add(runningInvoice.getInvoiceId());
            }
            List<InvoicesV1> oldInvoices = listInvoices.stream().filter(i -> !i.getInvoiceId().equalsIgnoreCase(runningInvoice.getInvoiceId())).toList();
            payableAmountForCurrentInvoiceRent = oldInvoices.stream().mapToDouble(i -> {
                if (i.getTotalAmount() != null) {
                    return i.getTotalAmount();
                }
                return 0.0;
            }).sum();
            currentMonthPaidRent = oldInvoices.stream().mapToDouble(i -> {
                if (i.getPaidAmount() != null) {
                    return i.getPaidAmount();
                }
                return 0.0;
            }).sum();
            double runningInvoicePaidAmount = 0.0;
            if (runningInvoice.getPaidAmount() != null) {
            ...
            double rentForRunningInvoice = rentPerDay * noOfDaysStayed;

            List<com.smartstay.smartstay.dao.InvoiceDiscounts> listInvoiceDiscounts = invoiceDiscountService.getInvoiceDiscounts(hostelId, discountedInvoiceIds);
            if (listInvoiceDiscounts != null) {
                discountAmount = listInvoiceDiscounts.stream().mapToDouble(com.smartstay.smartstay.dao.InvoiceDiscounts::getDiscountAmount).sum();
            }
            payableAmountForCurrentInvoiceRent = payableAmountForCurrentInvoiceRent + rentForRunningInvoice;
            ...
            if (listRentBreakup != null) {
                totalDaysStayed = listRentBreakup.stream().mapToLong(RentBreakUp::noOfDays).sum();
...
//        }
        ...
        List<String> invoiceIds = listInvoices.stream().map(InvoicesV1::getInvoiceId).toList();
        Double discountAmount = 0.0;
        double payableAmountForCurrentInvoiceRent = 0.0;
        double currentMonthPaidRent = 0.0;
        double currentMonthPayableAmount = 0.0;
        ...
        if (listInvoiceDiscounts != null) {
            isDiscountApplied.set(true);
            discountAmount = listInvoiceDiscounts.stream().mapToDouble(i -> {
...
//            List<com.smartstay.smartstay.dao.InvoiceDiscounts> listInvoiceDiscounts = invoiceDiscountService.getInvoiceDiscounts(hostelId, discountedInvoiceIds);
//            if (listInvoiceDiscounts != null) {
//                discountAmount = listInvoiceDiscounts
//                        .stream()
//                        .mapToDouble(com.smartstay.smartstay.dao.InvoiceDiscounts::getDiscountAmount)
//                        .sum();
//            }
            payableAmountForCurrentInvoiceRent = payableAmountForCurrentInvoiceRent + rentForRunningInvoice;
            currentMonthPaidRent = currentMonthPaidRent + runningInvoicePaidAmount;
            ...
            List<RentBreakUp> listRentBreakup = customersBedHistoryService.getBreakupBasedOnRentHistory(customers, leavingDate, billingDates);
            if (listRentBreakup != null) {
                totalNoOfDaysStayed = listRentBreakup.stream().mapToLong(RentBreakUp::noOfDays).sum();
...
//            OtherInvoicesInfo otherInvoicesInfo = getCurrentMonthOtherInvoices(customers.getHostelId(), customers.getCustomerId(), billingDates);
    ...
    public com.smartstay.smartstay.responses.settlement.UnpaidInvoices getUnpaidInvoicesInfo(String customerId, String hostelId, Date leavingDate) {
        BillingDates billingDates = hostelService.getCurrentBillStartAndEndDates(hostelId);
        List<String> invoicesTypes = new ArrayList<>();
        invoicesTypes.add(InvoiceType.RENT.name());
        invoicesTypes.add(InvoiceType.REASSIGN_RENT.name());
        invoicesTypes.add(InvoiceType.OTHER.name());
        List<InvoicesV1> listOldInvoices = invoicesV1Repository.findOldMonthPendingInvoices(customerId, billingDates.currentBillStartDate(), invoicesTypes);
        List<InvoicesV1> listCurrentMonthOtherInvoices = invoicesV1Repository.findCurrentMonthOtherInvoices(customerId, billingDates.currentBillStartDate());
        if (listOldInvoices != null) {
            if (listCurrentMonthOtherInvoices != null && !listCurrentMonthOtherInvoices.isEmpty()) {
                listOldInvoices.addAll(listCurrentMonthOtherInvoices);
            }
        }
        else {
            listOldInvoices = new ArrayList<>(listCurrentMonthOtherInvoices);
        }

        double unpaidAmount = listOldInvoices.stream().mapToDouble(i -> {
            if (i.getPaymentStatus().equalsIgnoreCase(PaymentStatus.PENDING.name())) {
                return i.getTotalAmount();
                ...
                return i.getTotalAmount() - paidAmount;
            }
            return 0.0;
        }).sum();
        double paidAmount = listOldInvoices.stream().mapToDouble(i -> {
            if (i.getPaymentStatus().equalsIgnoreCase(PaymentStatus.PARTIAL_PAYMENT.name())) {
                double pAmount = 0.0;
                if (i.getPaidAmount() != null) {
                    pAmount = i.getPaidAmount();
                }
        ...
        double totalAmount = listOldInvoices.stream().mapToDouble(InvoicesV1::getTotalAmount).sum();

        List<UnpaidInvoices> unpaidInvoices = listOldInvoices.stream().map(i -> new UnpaidInvoicesMapper().apply(i)).toList();

        return new com.smartstay.smartstay.responses.settlement.UnpaidInvoices(listOldInvoices.size(), Utils.roundOffWithTwoDigit(unpaidAmount), Utils.roundOffWithTwoDigit(totalAmount), Utils.roundOffWithTwoDigit(paidAmount), unpaidInvoices);
    }

    public com.smartstay.smartstay.responses.settlement.UnpaidInvoices getUnpaidInvoicesInfoForJoiningBased(String customerId, String hostelId, Date joiningDate, Date leavingDate) {
        BillingDates billingDates = hostelService.getJoiningBasedCurrentMonthBillingDate(joiningDate, hostelId, leavingDate);
        List<String> invoicesTypes = new ArrayList<>();
        invoicesTypes.add(InvoiceType.RENT.name());
        invoicesTypes.add(InvoiceType.REASSIGN_RENT.name());
        invoicesTypes.add(InvoiceType.OTHER.name());
        ...
        if (listOldInvoices != null) {
            ...
            if (listCurrentMonthOtherInvoices != null) {
                listOldInvoices = new ArrayList<>(listCurrentMonthOtherInvoices);
            }
        }
        double unpaidAmount = listOldInvoices.stream().mapToDouble(i -> {
    ...
    public ResponseEntity<?> editDiscount(String hostelId, String invoiceId, ApplyDiscount discount) {
        ...
        if (discount.discountAmount() != null && discount.discountAmount() > invoicesV1.getBasePrice()) {
            return new ResponseEntity<>(Utils.DISCOUNT_AMOUNT_VALIDATION, HttpStatus.BAD_REQUEST);
        }
        ...
        if ((invoicesV1.getTotalAmount() + isDiscountApplied.invoiceDifference()) == 0) {
            invoicesV1.setPaymentStatus(PaymentStatus.PAID.name());
        } else if (invoicesV1.getTotalAmount() + isDiscountApplied.invoiceDifference() < 0) {
            invoicesV1.setPaymentStatus(PaymentStatus.PENDING_REFUND.name());
        } else {
            invoicesV1.setPaymentStatus(PaymentStatus.PENDING.name());
        }
    ...
    public void modifyCurrentMonthDiscount(String customerId, String hostelId, double discountAmount, BillingDates billDate) {
        ...
        InvoicesV1 runningInvoice = invoicesV1Repository.findCurrentRunningInvoice(customerId, billDate.currentBillStartDate());
        InvoiceDiscountDto invoiceDiscountDto = invoiceDiscountService.modifyInvoiceDiscount(hostelId, runningInvoice.getInvoiceId(), discountAmount);
        if (invoiceDiscountDto != null) {
    ...
    public List<String> findUnpaidCurrentMonthInvoicesIds(String hostelId, String customerId, BillingDates billDate) {
        ...
        if (listCurrentMonthInvoice != null) {
            List<String> invoicesIds = listCurrentMonthInvoice.stream().filter(i -> i.getPaymentStatus().equalsIgnoreCase(PaymentStatus.PENDING.name()) || i.getPaymentStatus().equalsIgnoreCase(PaymentStatus.PARTIAL_PAYMENT.name())).map(InvoicesV1::getInvoiceId).toList();
            if (invoicesIds != null) {
                return invoicesIds;
            }
    ...
    public ResponseEntity<?> redeeemAdvanceAmount(String hostelId, String invoiceId, com.smartstay.smartstay.payloads.invoice.InvoiceRedemption invoiceRedemption) {
        ...
        if (invoiceRedemption.listItems() != null) {
            listRedemptions = invoiceRedemption.listItems().stream().filter(i -> i.invoiceId() != null && i.amount() != null).toList();

            redemptionAmount1 = invoiceRedemption.listItems().stream().mapToDouble(i -> {
                if (i.amount() != null) {
                    return i.amount();
                }
                return 0.0;
        ...
        List<String> targetInvoiceIds = listRedemptions.stream().map(RedemptionItems::invoiceId).toList();
        ...
        List<String> paidInvoices = listInvoices.stream().filter(i -> i.getPaymentStatus().equalsIgnoreCase(PaymentStatus.PAID.name())).map(InvoicesV1::getInvoiceId).toList();

        List<String> cancelledInvoices = listInvoices.stream().filter(InvoicesV1::isCancelled).map(InvoicesV1::getInvoiceId).toList();
        List<String> advanceInvoice = listInvoices.stream().filter(i -> i.getInvoiceType().equalsIgnoreCase(InvoiceType.ADVANCE.name()) || i.getInvoiceType().equalsIgnoreCase(InvoiceType.BOOKING.name())).map(InvoicesV1::getInvoiceId).toList();
        ...
        if (ir != null) {
            ...
            List<InvoicesV1> listNewInvoices = listInvoices.stream().map(i -> {
                if (i.getPaidAmount() != null) {
    ...
    public ResponseEntity<?> initializeRedemption(String hostelId, String advanceInvoiceId) {
        if (!authentication.isAuthenticated()) {
            return new ResponseEntity<>(Utils.UN_AUTHORIZED, HttpStatus.UNAUTHORIZED);
        }
        Users users = usersService.findUserByUserId(authentication.getName());
        ...
        InvoicesV1 advanceInvoices = invoicesV1Repository.findById(advanceInvoiceId).orElse(null);
        if (advanceInvoices == null) {
            return new ResponseEntity<>(Utils.INVALID_INVOICE_ID, HttpStatus.BAD_REQUEST);
        }
        if (!advanceInvoices.getHostelId().equalsIgnoreCase(hostelId)) {
            return new ResponseEntity<>(Utils.INVALID_REQUEST, HttpStatus.BAD_REQUEST);
        }
        ...
        if (listPendingInvoices != null) {
            List<String> invoiceIds = listPendingInvoices.stream().map(InvoicesV1::getInvoiceId).toList();

            listTransactions = transactionService.findLatestTransactionsByInvoiceIds(hostelId, invoiceIds);
            if (listTransactions != null) {
                Set<String> bankIds = listTransactions.stream().map(TransactionV1::getBankId).collect(Collectors.toSet());
                if (bankIds != null) {
        ...
        if (advanceInvoices.getPaymentStatus().equalsIgnoreCase(PaymentStatus.PENDING.name())) {
            status = false;
            message = "Cannot redeem this invoice";
        ...
        if (bookingsV1 != null) {
            bedDetails = bedService.getBedDetails(bookingsV1.getBedId());
        }
        if (bedDetails != null) {
            bedName = bedDetails.getBedName();
            floorName = bedDetails.getFloorName();
    ...
    public ResponseEntity<?> getAvailableInvoicesToApply(String hostelId, String invoiceId, String type) {
        ...
        SelectedInvoiceInfo selectedInvoiceInfo = new SelectedInvoiceInfo(invoicesV1.getInvoiceId(), invoicesV1.getInvoiceNumber(), invoicesV1.getPaidAmount(), pendingAmount, invoicesV1.getTotalAmount(), invoicesV1.getPaymentStatus(), invoicesV1.getInvoiceType(), Utils.dateToString(invoicesV1.getInvoiceStartDate()), Utils.dateToString(invoicesV1.getInvoiceDueDate()));
        ...
        List<com.smartstay.smartstay.responses.InvoiceRedemption.InvoiceInfo> invoiceInfoListArray = null;
        if (type != null && type.trim().equalsIgnoreCase("Credit")) {
            ...
            List<InvoicesV1> advanceInvoice = invoicesV1Repository.findAllAdvanceInvoices(invoicesV1.getCustomerId());
            InvoicesV1 existingAdvance = null;
            if (advanceInvoice != null) {
                invoiceInfoListArray = advanceInvoice
                        .stream()
    ...
    public AdvanceItems getRedeemedListFromAdvance(String hostelId, String customerId) {
        ...
        if (advanceInvoice == null) {
            return new AdvanceItems("Refundable Advance", InvoiceType.ADVANCE.name(), 0.0, 0.0, 0.0, "NA", null);
        }
        if (advanceInvoice.getPaymentStatus().equalsIgnoreCase(PaymentStatus.PENDING.name())) {
            return new AdvanceItems("Refundable Advance", InvoiceType.ADVANCE.name(), 0.0, 0.0, 0.0, advanceInvoice.getInvoiceNumber(), null);
        }
        ...
        return new AdvanceItems("Refundable Advance", advanceInvoice.getInvoiceType(), availableAmount, paidAmount - availableAmount, paidAmount, advanceInvoice.getInvoiceNumber(), listRedeemedInfo);
    }

    public List<AdvanceItems> getRedeemedListFromOtherAdvance(String hostelId, String customerId) {
        List<InvoicesV1> advanceInvoice = invoicesV1Repository.findAdditionalAdvanceInvoiceByCustomerId(customerId);
        if (advanceInvoice == null) {
            return new ArrayList<>();
        }

        List<String> advanceIds = advanceInvoice
                .stream()
                .map(InvoicesV1::getInvoiceId)
                .toList();

        Double paidAmount = advanceInvoice
                .stream()
                .mapToDouble(i -> {
                    if (i.getPaymentStatus().equalsIgnoreCase(PaymentStatus.PAID.name()) || i.getPaymentStatus().equalsIgnoreCase(PaymentStatus.PARTIAL_PAYMENT.name())) {
                        if (i.getPaidAmount() != null) {
                ...
                .sum();
        Double availableAmount = advanceInvoice
                .stream()
                .mapToDouble(i -> {
                    if (i.getPaymentStatus().equalsIgnoreCase(PaymentStatus.PAID.name()) || i.getPaymentStatus().equalsIgnoreCase(PaymentStatus.PARTIAL_PAYMENT.name())) {
                        if (i.getBalanceAmount() != null && i.getBalanceAmount() > 0) {
...
//        }
        ...
        List<RedeemedInfo> listRedeemedInfo = invoiceRedemptionService.findRedeemedItemsFromAdvance(hostelId, advanceIds);
        List<AdvanceItems> listAdvanceItems = advanceInvoice
                .stream()
                .map(i -> {
                    List<RedeemedInfo> redeemedInfos = listRedeemedInfo
                            .stream()
                            .filter(i2 -> i2.invoiceId().equalsIgnoreCase(i.getInvoiceId()))
                            .toList();
                    return new AdvanceItems("Refundable Advance", i.getInvoiceType(), availableAmount, paidAmount - availableAmount, paidAmount, i.getInvoiceNumber(), redeemedInfos);
                })
                .toList();

        return listAdvanceItems;
    }
    ...
    public AdvanceItems getRedeemedListFromBookings(String hostelId, String customerId) {
        ...
        if (advanceInvoice == null) {
            return new AdvanceItems("Refundable Bookings", null, 0.0, 0.0, 0.0, "NA", null);
        }
        if (advanceInvoice.getPaymentStatus().equalsIgnoreCase(PaymentStatus.PENDING.name())) {
            return new AdvanceItems("Refundable Bookings", advanceInvoice.getInvoiceType(), 0.0, 0.0, 0.0, advanceInvoice.getInvoiceNumber(), null);
        }
        ...
        return new AdvanceItems("Refundable Bookings",advanceInvoice.getInvoiceType(), availableAmount, paidAmount - availableAmount, paidAmount, advanceInvoice.getInvoiceNumber(), listRedeemedInfo);
    }
    ...
    public ResponseEntity<?> getAdvanceInvoicesForRedemption(String hostelId, String name, String period, List<String> type, String status, String startDate, String endDate, String floor, String room, Integer minAmount, Integer maxAmount, int page, int size) {
        ...
        return getBookingResponse(hostelId, name, period, type, status, startDate, endDate, floor, room, minAmount, maxAmount, page, size);
    }
    ...
    private ResponseEntity<?> getBookingResponse(String hostelId, String name, String period, List<String> type, String status, String startDate, String endDate, String floor, String room, Integer minAmount, Integer maxAmount, int page, int size) {
        ...
        if (period != null && !period.isEmpty()) {
            if (period.equalsIgnoreCase("TODAY")) {
                ...
                dStartDate = Utils.getStartOfCurrentWeek();
                dEndDate = new Date();
            }
                ...
                if (billingDates != null) {
                    ...
                    dEndDate = billingDates.currentBillEndDate();
                }
            }
        }
        ...
        if (!listFloor.isEmpty()) {
            floorsFilterItem = listFloor.stream().map(i -> new BookingsFilterOptions.FilterItems(i.getFloorName(), String.valueOf(i.getFloorId()))).toList();
        }

        if (!roomsList.isEmpty()) {
            roomsFilterItem = roomsList.stream().map(i -> new BookingsFilterOptions.RoomsItem(i.getRoomName(), String.valueOf(i.getRoomId()), String.valueOf(i.getFloorId()))).toList();
        }

        List<BookingsFilterOptions.FilterItems> invoiceTypesFilterOptions = new ArrayList<>();
        invoiceTypesFilterOptions.add(new BookingsFilterOptions.FilterItems("Rent", InvoiceType.AMOUNT_HOLDING.name()));
        invoiceTypesFilterOptions.add(new BookingsFilterOptions.FilterItems("EB", InvoiceType.EB_HOLDING.name()));
        invoiceTypesFilterOptions.add(new BookingsFilterOptions.FilterItems("Booking", InvoiceType.BOOKING.name()));
        invoiceTypesFilterOptions.add(new BookingsFilterOptions.FilterItems("Advance", InvoiceType.ADVANCE.name()));
        invoiceTypesFilterOptions.add(new BookingsFilterOptions.FilterItems("Additional Advance", InvoiceType.ADDITIONAL_ADVANCE.name()));
        ...
        invoiceStatusFilterOptions.add(new BookingsFilterOptions.FilterItems("Partially Adjusted", "PARTIALLY_REDEEMED"));
        invoiceStatusFilterOptions.add(new BookingsFilterOptions.FilterItems("Fully Adjusted", "FULLY_REDEEMED"));
        ...
        listPeriods.add(new BookingsFilterOptions.FilterItems("This Week", "THIS_WEEK"));
        listPeriods.add(new BookingsFilterOptions.FilterItems("This Month", "THIS_MONTH"));

        BookingsFilterOptions bookingsFilterOptions = new BookingsFilterOptions(floorsFilterItem, roomsFilterItem, invoiceTypesFilterOptions, invoiceStatusFilterOptions, listPeriods);
        ...
        if (listAllCheckedInCustomers != null && !listAllCheckedInCustomers.isEmpty()) {
            List<String> listCustomerIds = listAllCheckedInCustomers.stream().map(BookingsV1::getCustomerId).toList();

            List<Customers> listCustomers = customersService.getCustomerDetails(listCustomerIds, name);

            List<String> filteredIds = listCustomers.stream().map(Customers::getCustomerId).toList();
            ...
            if (authentication.getSource().equalsIgnoreCase("Web")) {
                Page<InvoicesV1> pagebleAdvances = invoicesV1Repository.findPaidAdvanceInvoicesForRedemption(hostelId, filteredIds, invoiceTypes, dStartDate, dEndDate, status, minBalance, maxBalance, pageableRequest);
        ...
        return new ResponseEntity<>(Utils.NO_BOOKINGS_FOUND, HttpStatus.OK);
    }
    ...
    public double getAdvanceAmountFromBookingInvoice(String hostelId, List<String> invoiceIds) {
        return invoiceRedemptionService.getAdvanceAmountFromBookingInvoice(hostelId, invoiceIds);
    }
    ...
    public List<String> getInvoiceNumbersBySearchKeyword(String hostelId, String keyword, List<String> invoiceType, List<String> customerIds) {
        ...
        return listInvoices.stream().map(InvoicesV1::getInvoiceId).toList();
    }

    public ResponseEntity<?> getBasicList(String hostelId, List<String> cusIds, String startDate, String endDate, List<String> types, List<String> createdBy, List<String> invoiceModes, String searchKey, List<String> paymentStatus) {
        if (!authentication.isAuthenticated()) {
            return new ResponseEntity<>(Utils.UN_AUTHORIZED, HttpStatus.UNAUTHORIZED);
        }
        Users users = usersService.findUserByUserId(authentication.getName());
        ...
        if (authentication.isAuthenticated()) {
            ...
            if (templates != null) {
                ...
                if (templates.gstPercentile() != null) {
                    ...
                    cgst = templates.gstPercentile() / 2;
                    sgst = templates.gstPercentile() / 2;
                    baseAmount = rentAmount / (1 + (templates.gstPercentile() / 100));
...
//            Date joiningDate1 = Utils.stringToDate(joiningDate.replace("/", "-"), Utils.USER_INPUT_DATE_FORMAT);
            Date dueDate = Utils.addDaysToDate(currentMonthBillingDates.currentBillStartDate(), currentMonthBillingDates.dueDays() - 1);
            Date endDate = currentMonthBillingDates.currentBillEndDate();

            invoicesV1.setTotalAmount(rentAmount);
            invoicesV1.setBasePrice(baseAmount);
            invoicesV1.setSubTotal(rentAmount);
            invoicesV1.setDeductionAmount(0.0);
            invoicesV1.setDeductions(null);
            invoicesV1.setBalanceAmount(0.0);
            invoicesV1.setInvoiceType(InvoiceType.RENT.name());
            ...
            invoicesV1.setCustomerMailId(customerMailId);
            invoicesV1.setCreatedAt(new Date());
            invoicesV1.setInvoiceDate(currentMonthBillingDates.currentBillStartDate());
            invoicesV1.setInvoiceStartDate(currentMonthBillingDates.currentBillStartDate());
            invoicesV1.setInvoiceEndDate(Utils.convertToTimeStamp(endDate));
            invoicesV1.setInvoiceGeneratedDate(new Date());
            invoicesV1.setInvoiceMode(InvoiceMode.AUTOMATIC.name());
            invoicesV1.setCancelled(false);
            ...
            PaymentSummary summary = new PaymentSummary(hostelId, customerId, invoiceNumber.toString(), rentAmount, customerMailId, customerMobile, "Active");
            paymentSummaryService.addInvoice(summary);
        }
    }

    public ResponseEntity<?> initializeRecordPayment(String hostelId, String invoiceId) {
        if (!authentication.isAuthenticated()) {
            return new ResponseEntity<>(Utils.UN_AUTHORIZED, HttpStatus.UNAUTHORIZED);
        }
        Users users = usersService.findUserByUserId(authentication.getName());
        ...
        if (invoicesV1.getPaymentStatus().equalsIgnoreCase(PaymentStatus.PAID.name())) {
            return new ResponseEntity<>(Utils.INVOICE_ALREADY_PAID, HttpStatus.BAD_REQUEST);
        }
        Integer bedId = 0;
        Double paidAmount = 0.0;
        Double pendingAmount = 0.0;
        ...
        BookingsV1 bookingsV1 = bookingsService.getBookingsByCustomerId(invoicesV1.getCustomerId());
        if (bookingsV1 != null) {
            bedId = bookingsV1.getBedId();
        }
        CustomerDetails customerInfo = customersService.getCustomerInformationForRecordPayment(invoicesV1.getCustomerId());
        StayInfo stayInfo = bedService.getBedInformationGeneral(bedId);
        List<BankInfoRecordPayments> bankInfo = bankingService.getBankForReceivingPayments(invoicesV1.getHostelId());
        RecordPayment recordPayment = new RecordPayment(invoicesV1.getInvoiceId(),
    ...
    public ResponseEntity<?> initializeInvoiceDiscounts(String hostelId, String invoiceId) {
        if (!authentication.isAuthenticated()) {
            return new ResponseEntity<>(Utils.UN_AUTHORIZED, HttpStatus.UNAUTHORIZED);
        }
        Users users = usersService.findUserByUserId(authentication.getName());
        ...
        if (invoicesV1.getBasePrice() != null) {
            invoiceAMount = invoicesV1.getBasePrice();
        }
        long overdueDays = Utils.findNumberOfDays(invoicesV1.getInvoiceDueDate(), new Date()) - 1;
        if (overdueDays < 0) {
        ...
        com.smartstay.smartstay.responses.discount.InvoiceInfo invoiceInfo = new com.smartstay.smartstay.responses.discount.InvoiceInfo(invoicesV1.getInvoiceId(),
                invoicesV1.getInvoiceNumber(),
                invoiceDate,
                invoiceAMount,
                paidAmount,
        ...
        com.smartstay.smartstay.responses.discount.CustomerInfo customerInfo = customersService.getCustomerInfoForDiscount(invoicesV1.getCustomerId());
        BedDetails bedDetails = null;
        BookingsV1 bookingsV1 = bookingsService.getBookingsByCustomerId(invoicesV1.getCustomerId());
        if (bookingsV1 != null) {
            bedDetails = bedService.getBedDetails(bookingsV1.getBedId());
        }
        com.smartstay.smartstay.responses.discount.StayInfo stayInfo = null;
        if (bedDetails != null) {
            stayInfo = new com.smartstay.smartstay.responses.discount.StayInfo(bedDetails.getBedName(),
                    bedDetails.getFloorName(), bedDetails.getRoomName());
        }
    ...
    public ResponseEntity<?> addManualInvoice(String hostelId, String customerId, ManualInvoiceNew manualInvoiceBody) {
        if (!authentication.isAuthenticated()) {
            return new ResponseEntity<>(Utils.UN_AUTHORIZED, HttpStatus.UNAUTHORIZED);
        }
        Users users = usersService.findUserByUserId(authentication.getName());
        ...
        if (customers.getCurrentStatus().equalsIgnoreCase(CustomerStatus.BOOKED.name())) {
            return new ResponseEntity<>(Utils.CANNOT_CREATE_INVOICE_BOOKIED_CUSTOMER, HttpStatus.BAD_REQUEST);
        }
        if (customers.getCurrentStatus().equalsIgnoreCase(CustomerStatus.DRAFT.name())) {
            return new ResponseEntity<>(Utils.CANNOT_CREATE_INVOICE_DRAFTED_CUSTOMER, HttpStatus.BAD_REQUEST);
        }
        if (customers.getCurrentStatus().equalsIgnoreCase(CustomerStatus.SETTLEMENT_GENERATED.name())) {
            return new ResponseEntity<>(Utils.CANNOT_CREATE_INVOICE_SETTLEMENT_CUSTOMERS, HttpStatus.BAD_REQUEST);
        }
        if (customers.getCurrentStatus().equalsIgnoreCase(CustomerStatus.VACATED.name())) {
            return new ResponseEntity<>(Utils.CANNOT_CREATE_INVOICE_VACATED_CUSTOMERS, HttpStatus.BAD_REQUEST);
        }
        if (customers.getCurrentStatus().equalsIgnoreCase(CustomerStatus.CANCELLED_BOOKING.name())) {
            return new ResponseEntity<>(Utils.CANNOT_CREATE_CANCELLED_BOOKING_CUSTOMERS, HttpStatus.BAD_REQUEST);
        }
        if (customers.getCurrentStatus().equalsIgnoreCase(CustomerStatus.DELETED.name())) {
            return new ResponseEntity<>(Utils.CANNOT_CREATE_DELETED_CUSTOMERS, HttpStatus.BAD_REQUEST);
        }
        if (manualInvoiceBody == null) {
            return new ResponseEntity<>(Utils.PAYLOADS_REQUIRED, HttpStatus.BAD_REQUEST);
        }
        if (manualInvoiceBody.invoiceItems() == null) {
            return new ResponseEntity<>(Utils.REQUIRED_INVOICE_ITEMS, HttpStatus.BAD_REQUEST);
        }
        BookingsV1 bookingsV1 = bookingsService.getBookingsByCustomerId(customerId);
        if (bookingsV1 == null) {
            return new ResponseEntity<>(Utils.INVALID_CUSTOMER_ID, HttpStatus.BAD_REQUEST);
        }
        String paymentStatus = PaymentStatus.PENDING.name();
        Date invoiceDate = new Date();
        if (manualInvoiceBody.invoiceDate() != null && !manualInvoiceBody.invoiceDate().trim().isEmpty()) {
            invoiceDate = Utils.stringToDate(manualInvoiceBody.invoiceDate().replaceAll("/", "-"), Utils.USER_INPUT_DATE_FORMAT);
        }

        if (Utils.compareWithTwoDates(invoiceDate, bookingsV1.getJoiningDate()) < 0) {
            return new ResponseEntity<>(Utils.MANUAL_INVOICE_CANNOT_CREATE_BEFORE_CHECKIN, HttpStatus.BAD_REQUEST);
        }
        if (Utils.compareWithTwoDates(invoiceDate, new Date()) > 0) {
            return new ResponseEntity<>(Utils.FUTURE_DATES_NOT_ALLOWED, HttpStatus.BAD_REQUEST);
        }
        BillingDates billingDates = hostelService.getBillingRuleOnDate(hostelId, invoiceDate);
        Date dueDate = Utils.addDaysToDate(invoiceDate, billingDates.dueDays() - 1);
        ItemResponse rentalInvoice = manualInvoiceBody.invoiceItems()
                .stream()
                .filter(i -> i.invoiceItem().equalsIgnoreCase(InvoiceType.RENT.name()))
                .findFirst()
                .orElse(null);
        ItemResponse advanceInvoice = manualInvoiceBody.invoiceItems()
                .stream()
        ...
        if (manualInvoiceBody.invoiceNumber() != null && !manualInvoiceBody.invoiceNumber().isEmpty()) {
            if (invoicesV1Repository.findByInvoiceNumberAndHostelId(manualInvoiceBody.invoiceNumber(), customers.getHostelId()) != null) {
                ...
                invoiceNumber = manualInvoiceBody.invoiceNumber();
            }
        }

        String invoiceType = null;

        if (rentalInvoice != null && advanceInvoice != null) {
            return new ResponseEntity<>(Utils.CANNOT_CREATE_ADVANCE_RENTAL_TOGETHER, HttpStatus.BAD_REQUEST);
        }
        if (rentalInvoice != null) {
            if (billingDates.billingModel().equalsIgnoreCase(BillingModel.POSTPAID.name())) {
                BillingDates currentMonthBillingDates = hostelService.getBillingRuleOnDate(hostelId, new Date());
                if (currentMonthBillingDates != null) {
                    if (Utils.compareWithTwoDates(invoiceDate, currentMonthBillingDates.currentBillStartDate()) >= 0) {
                        return new ResponseEntity<>(Utils.CANNOT_CREATE_CURRENT_MONTH_INVOICE_POSTPAID, HttpStatus.BAD_REQUEST);
                    }
                }

            }
            List<InvoicesV1> rentalInvoices = invoicesV1Repository.findInvoiceByCustomerIdAndDate(customerId, billingDates.currentBillStartDate(), billingDates.currentBillEndDate());
            if (rentalInvoices != null && !rentalInvoices.isEmpty()) {
                return new ResponseEntity<>(Utils.INVOICE_ALREADY_PRESENT, HttpStatus.BAD_REQUEST);
            }
            invoiceType = InvoiceType.RENT.name();
            paymentStatus = PaymentStatus.PAID.name();
        } else if (advanceInvoice != null) {
            invoiceType = InvoiceType.ADDITIONAL_ADVANCE.name();
        } else {
            invoiceType = InvoiceType.OTHER.name();
        }

        if (manualInvoiceBody.invoiceNumber() == null || manualInvoiceBody.invoiceNumber().isEmpty()) {
            String invType = invoiceType;
            if (invoiceType.equalsIgnoreCase(InvoiceType.OTHER.name())) {
                invType = InvoiceType.RENT.name();
            }
            invoiceNumber = generateInvoiceNumber(hostelId, invType);
        }
        ...
        if (manualInvoiceBody.invoiceItems() != null && !manualInvoiceBody.invoiceItems().isEmpty()) {
            invoiceAmount = manualInvoiceBody.invoiceItems()
                    .stream()
                    .mapToDouble(i -> {
                        if (i.amount() == null) {
                            return 0.0;
...
//            }
        ...
        invoicesV1.setSgst(0.0);
        invoicesV1.setGstPercentile(0.0);
        invoicesV1.setPaymentStatus(paymentStatus);
        invoicesV1.setDeductionAmount(0.0);
        invoicesV1.setOthersDescription(null);
        invoicesV1.setInvoiceMode(InvoiceMode.MANUAL.name());
        invoicesV1.setCancelled(false);
        invoicesV1.setDiscounted(isDiscounted);
        invoicesV1.setCancelledInvoices(null);
        invoicesV1.setNewCancelledInvoices(null);
        invoicesV1.setDeductions(null);
        invoicesV1.setInvoiceUrl(null);
        invoicesV1.setCreatedBy(authentication.getName());
        invoicesV1.setUpdatedBy(authentication.getName());
        invoicesV1.setInvoiceGeneratedDate(invoiceDate);
        invoicesV1.setCancelledDate(null);
        invoicesV1.setInvoiceDueDate(Utils.convertToTimeStamp(dueDate));
        invoicesV1.setInvoiceDate(Utils.convertToTimeStamp(invoiceDate));
        invoicesV1.setInvoiceStartDate(Utils.convertToTimeStamp(invoiceDate));
        invoicesV1.setInvoiceEndDate(Utils.convertToTimeStamp(billingDates.currentBillEndDate()));
        invoicesV1.setCreatedAt(new Date());
        invoicesV1.setUpdatedAt(new Date());

        List<InvoiceItems> invoiceItems = manualInvoiceBody
                .invoiceItems()
                .stream()
                .map(i -> {
                    InvoiceItems invItm = new InvoiceItems();
                    invItm.setAmount(i.amount());
                    invItm.setInvoice(invoicesV1);
                    if (i.invoiceItem().equalsIgnoreCase(com.smartstay.smartstay.ennum.InvoiceItems.RENT.name())) {
                        invItm.setInvoiceItem(com.smartstay.smartstay.ennum.InvoiceItems.RENT.name());
                    } else if (i.invoiceItem().equalsIgnoreCase(com.smartstay.smartstay.ennum.InvoiceItems.ADVANCE.name())) {
                        invItm.setInvoiceItem(com.smartstay.smartstay.ennum.InvoiceItems.ADVANCE.name());
                    } else if (i.invoiceItem().equalsIgnoreCase(com.smartstay.smartstay.ennum.InvoiceItems.MAINTENANCE.name())) {
                        invItm.setInvoiceItem(com.smartstay.smartstay.ennum.InvoiceItems.MAINTENANCE.name());
                    } else {
                        invItm.setInvoiceItem(com.smartstay.smartstay.ennum.InvoiceItems.OTHERS.name());
                        invItm.setOtherItem(i.invoiceItem());
                    }
        ...
        InvoicesV1 inv1 = invoicesV1Repository.save(invoicesV1);
        if (invoiceType.equalsIgnoreCase(InvoiceType.ADDITIONAL_ADVANCE.name())) {
            Advance advance = customers.getAdvance();
            if (advance == null) {
                advance = new Advance();
            }
            double existingAmount = advance.getAdvanceAmount();
            advance.setAdvanceAmount(existingAmount + invoiceAmount);
        ...
        usersService.addUserLog(hostelV1.getHostelId(), inv1.getInvoiceId(), ActivitySource.INVOICE, ActivitySourceType.MANUAL_BILL, users);
        return new ResponseEntity<>(HttpStatus.CREATED);
    }

    public ReportsDto getCurrentMonthInvoiceDetailsForReports(String hostelId, Date startDate, Date endDate) {
        ...
        if (listInvoices != null) {
            ...
            paidAmount = listInvoices
                    ...
                    .sum();
            outstandingAmount = listInvoices
                    .stream()
                    .mapToDouble(i -> {
                        double pAmount = 0.0;
                        double tAmount = 0.0;
                        ...
                        if (i.getTotalAmount() != null) {
                            tAmount = i.getTotalAmount();
                        }
                        return tAmount - pAmount;
                    })
                    .sum();

            return new ReportsDto(Utils.roundOffWithTwoDigit(paidAmount),
                    Utils.roundOffWithTwoDigit(outstandingAmount),
                    Utils.roundOffWithTwoDigit(totalAmount),
                    listInvoices.size());
        }
    ...
    public List<String> findInvoicesByInvoiceIdAndTypes(String hostelId, List<String> invoiceId, List<String> invoiceTypeArr) {
        List<InvoicesV1> listInvoices = invoicesV1Repository.findByInvoiceIdAndInvoiceType(hostelId, invoiceId, invoiceTypeArr);
        if (listInvoices != null) {
            return listInvoices
                    .stream()
                    .map(InvoicesV1::getInvoiceId)
    ...
    public AdditionalAdvances getAdditionalAdvances(String hostelId, String customerId) {
        List<InvoicesV1> additionalInvoices = invoicesV1Repository.findAdditionalAdvanceInvoiceByCustomerId(customerId);
        AdditionalAdvances additionalAdvances = null;
        if (additionalInvoices != null && !additionalInvoices.isEmpty()) {
            double totalAdvanceAmount = additionalInvoices
                    .stream()
                    .mapToDouble(i -> {
                        if (i.getPaidAmount() != null) {
                            return i.getPaidAmount();
                    ...
                    .sum();
            double paidAmount = additionalInvoices
                    .stream()
                    ...
                    .sum();
            double balanceAmount = additionalInvoices
                    .stream()
                    .filter(i -> i.getPaymentStatus().equalsIgnoreCase(PaymentStatus.PAID.name()) || i.getPaymentStatus().equalsIgnoreCase(PaymentStatus.PARTIAL_PAYMENT.name()))
                    .mapToDouble(i -> {
                        if (i.getBalanceAmount() != null) {
                    ...
                    .sum();
            List<AdditionalAdvanceItems> additionalAdvanceItems = additionalInvoices
                    .stream()
                    .map(i -> {
                        double invoiceBalance = 0.0;
                        double pAmount = 0.0;
                        if (i.getBalanceAmount() != null) {
                            invoiceBalance = i.getBalanceAmount();
                        }
                        if (i.getPaidAmount() != null) {
                            pAmount = i.getPaidAmount();
                        }
