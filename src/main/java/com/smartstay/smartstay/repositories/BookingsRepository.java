...
public interface BookingsRepository extends JpaRepository<BookingsV1, String> {
    ...
    @Query(value = """
            SELECT * FROM bookingsv1 where hostel_id=:hostelId AND customer_id IN (:customerIds) AND current_status IN ('CHECKIN', 'NOTICE', 'BOOKED')
            """, nativeQuery = true)
    ...
    @Query(value = """
            ...
            """, nativeQuery = true)
    List<BookingsV1> findCheckedCustomersByListOfCustomersAndHostelId(List<String> customerIds, String hostelId);

    @Query(value = """
            SELECT * FROM bookingsv1 where hostel_id=:hostelId  AND current_status IN ('CHECKIN', 'NOTICE')
            """, nativeQuery = true)
    ...
    @Query("""
            SELECT booking FROM BookingsV1 booking WHERE booking.hostelId=:hostelId AND booking.currentStatus IN (
            'NOTICE', 'CHECKIN')
            """)
    ...
    @Query("""
            ...
            ((booking.joiningDate IS NOT NULL AND DATE(booking.joiningDate) <= DATE(:endDate) AND DATE(booking.joiningDate) >= DATE(:startDate)) 
            OR (booking.joiningDate IS NULL AND DATE(booking.expectedJoiningDate) <= DATE(:endDate) AND DATE(booking.expectedJoiningDate) >= DATE(:startDate)))
            """)
