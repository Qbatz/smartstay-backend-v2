...
public interface RentHistoryRepository extends JpaRepository<RentHistory, Long> {
    @Query(value = """
            SELECT * FROM rent_history WHERE customer_id=:customerId AND is_active = true limit 1
            """, nativeQuery = true)
    RentHistory findByCustomerId(@Param("customerId") String customerId);

    @Query(value = """
            SELECT * FROM rent_history WHERE customer_id=:customerId AND DATE(starts_from) <= DATE(:date) AND is_active = true ORDER by starts_from DESC LIMIT 1
            """, nativeQuery = true)
    ...
    @Query(value = """
            ...
            (ending_at IS NULL OR DATE(ending_at) >= :startDate) AND is_active = true
    ...
    @Query(value = """
            SELECT * FROM `rent_history` WHERE DATE(starts_from) = DATE(:startsFrom) AND is_active = true
            """, nativeQuery = true)
    List<RentHistory> findRentApplyFromDate(@Param("startsFrom") Date startsFrom);

    @Query(value = """
            SELECT * FROM rent_history WHERE customer_id = :customerId AND DATE(starts_from) >= DATE(:startsFrom) AND is_active = true limit 1
            """, nativeQuery = true)
    RentHistory findByCustomerIdAndStartsFrom(@Param("customerId") String customerId, @Param("startsFrom") Date startsFrom);

    @Query(value = """
            SELECT * FROM rent_history WHERE customer_id = :customerId AND DATE(starts_from) > DATE(:date) AND is_active = true ORDER BY starts_from ASC LIMIT 1
            """, nativeQuery = true)
