...
public interface InvoicesV1Repository extends JpaRepository<InvoicesV1, String> {
    ...
    AND (:startDate IS NULL OR i.invoiceStartDate >= :startDate)
    AND (:endDate IS NULL OR i.invoiceEndDate <= :endDate)
    ...
    """)
    List<InvoicesV1> findAllInvoicesByHostelIdForInvoice(@Param("hostelId") String hostelId, @Param("startDate") Date startDate, @Param("endDate") Date endDate, @Param("types") List<String> types, @Param("createdBy") List<String> createdBy, @Param("mode") List<String> mode, @Param("paymentStatus") List<String> paymentStatus, @Param("userId") List<String> userId, @Param("searchKey") String searchKey);
    ...
    @Query(value = """
            ...
            ORDER BY i.invoiceStartDate DESC
            """)
    Page<InvoicesV1> findAllInvoicesByHostelIdForHostelId(@Param("hostelId") String hostelId, @Param("startDate") Date startDate, @Param("endDate") Date endDate, @Param("customerIds") List<String> customerIds, @Param("types") List<String> types, @Param("createdBy") List<String> createdBy, @Param("mode") List<String> mode, @Param("paymentStatus") List<String> paymentStatus, Double minPaidAmount, Double maxPaidAmount, Double minOutstandingAmount, Double maxOutstandingAmount, Pageable pageable);
    @Query(value = """
    ...
    @Query("""
            SELECT i FROM InvoicesV1 i WHERE i.hostelId=:hostelId AND i.customerId=:customerId AND 
            i.invoiceType IN ('RENT', 'REASSIGN_RENT', 'ADDITIONAL_ADVANCE', 'OTHER') AND DATE(i.invoiceStartDate) <= DATE(:currentMonthStartDate)
            """)
    List<InvoicesV1> findAllInvoicesExceptCurrentMonth(String customerId, String hostelId, Date currentMonthStartDate);

    @Query("""
            SELECT i FROM InvoicesV1 i WHERE i.hostelId=:hostelId AND i.customerId=:customerId AND 
            i.invoiceType IN ('ADDITIONAL_ADVANCE', 'OTHER') AND DATE(i.invoiceStartDate) >= DATE(:currentMonthStartDate)
            """)
    List<InvoicesV1> findCurrentMonthOtherInvoices(String customerId, String hostelId, Date currentMonthStartDate);
    @Query(value = """
            SELECT * FROM invoicesv1 WHERE customer_id=:customerId AND hostel_id=:hostelId AND DATE(invoice_start_date) <= DATE(:startDate)
    ...
    @Query("""
            ...
            ORDER BY i.invoiceStartDate DESC
            """)
    List<InvoicesV1> findInvoicesByFilters(@Param("hostelId") String hostelId, @Param("startDate") Date startDate, @Param("endDate") Date endDate, @Param("customerIds") List<String> customerId, @Param("paymentStatus") List<String> paymentStatus, @Param("invoiceModes") List<String> invoiceModes, @Param("invoiceTypes") List<String> invoiceTypes, @Param("createdBy") List<String> createdBy, @Param("minPaidAmount") Double minPaidAmount, @Param("maxPaidAmount") Double maxPaidAmount, @Param("minOutstandingAmount") Double minOutstandingAmount, @Param("maxOutstandingAmount") Double maxOutstandingAmount);
    ...
    @Query("""
            SELECT i FROM InvoicesV1 i WHERE i.customerId =:customerId AND i.invoiceType='OTHER' 
            AND i.paymentStatus IN ('PENDING', 'PARTIAL_PAYMENT') 
            AND i.isCancelled=false AND DATE(i.invoiceStartDate) >= DATE(:currentMonth)
            """)
    List<InvoicesV1> findCurrentMonthOtherInvoices(String customerId, Date currentMonth);
    ...
    @Query("""
            SELECT i FROM InvoicesV1 i WHERE i.hostelId=:hostelId AND i.invoiceType IN (:invoiceTypes) 
            AND i.isCancelled=false AND i.balanceAmount > 0
            """)
    ...
    @Query(value = """
            ...
            OR (:status = 'FULLY_REDEEMED' AND i.balanceAmount = 0)
            ) 
    ...
    @Query(value = """
            ...
            bed.bedId = b.bedId AND r.roomId = b.roomId AND f.floorId = b.floorId AND 
            i.hostelId=:hostelId AND 
            i.customerId IN (:customerIds) AND i.invoiceType IN (:invoiceTypes) AND 
            i.paymentStatus in ('PAID', 'PARTIAL_PAYMENT') AND i.isCancelled=false AND 
            (:startDate IS NULL OR DATE(i.invoiceStartDate) >= DATE(:startDate)) AND 
            ...
            (:maxAmount IS NULL OR i.balanceAmount <= :maxAmount) 
            """)
    Page<InvoicesV1> findPaidAdvanceInvoicesForRedemption(String hostelId, List<String> customerIds, List<String> invoiceTypes,  Date startDate, Date endDate, String status, Integer minAmount, Integer maxAmount, Pageable pageable);
    ...
    @Query("""
            SELECT i FROM InvoicesV1 i WHERE i.isCancelled=TRUE
            """)
    List<InvoicesV1> findAllCancelledInvoice();
    @Query("""
