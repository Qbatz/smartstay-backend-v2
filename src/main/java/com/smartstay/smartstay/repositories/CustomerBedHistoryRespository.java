...
public interface CustomerBedHistoryRespository extends JpaRepository<CustomersBedHistory, Long> {
    ...
    @Query(value = """
            ...
            """, nativeQuery = true)
    CustomersBedHistory findPreviousCustomerOnBed(@Param("bedId") int bedId, @Param("customerId") String customerId);
    ...
    @Query("""
            SELECT cbh FROM CustomersBedHistory cbh WHERE cbh.customerId=:customerId AND cbh.type IN ('CHECK_IN', 'REASSIGNED', 'RENT_REVISION') 
    ...
    @Query(value = """
            ...
            WHERE bed_id = :bedId AND customer_id != :customerId
            AND type IN ('CHECK_IN', 'REASSIGNED')
            AND end_date IS NOT NULL AND DATE(end_date) > DATE(:joiningDate)
            """, nativeQuery = true)
