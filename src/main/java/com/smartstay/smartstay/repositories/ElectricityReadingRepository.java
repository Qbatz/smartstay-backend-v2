...
public interface ElectricityReadingRepository extends JpaRepository<com.smartstay.smartstay.dao.ElectricityReadings, Integer> {
    ...
    @Query(value = """
            ...
            and e2.bill_end_date <= DATE(:endDate) AND  e2.is_first_entry=false) as consumption, 
    ...
    @Query(value = """
            ...
            AND DATE(bill_start_date) <= DATE(:endDate) AND DATE(bill_end_date) >= DATE(:startDate) AND bill_status='INVOICE_NOT_GENERATED'
            """, nativeQuery = true)
